...
inline Void GameForm::makegrid(int Gridsz)
{
	...
	array<Point>^ arr = gcnew array<Point>(40);
	int k = 0;
...
inline Void GameForm::explose(bomb^ cur_bomb)
{
	...
	if (e_pict[b_p->Y][b_p->X] != obj::exit)
		changepic(Point(b_p->X, b_p->Y), obj::fire);
...
inline Void GameForm::try_explose(Point vec, int len, Point^ from)
{
	if (vec.X > 0)
	{
		for (int i = 1; i <= len; ++i)
		{
			...
			if (e_pict[from->Y][from->X + i] == obj::brick || i == len)
			{
	...
	if (vec.Y > 0)
	{
		for (int i = 1; i <= len; ++i)
		{
			...
			if (e_pict[from->Y + i][from->X] == obj::brick || i == len)
			{
...
inline Void GameForm::init_timer_chk()
{
	...
	timer_chk->Interval = 50;
	timer_chk->Tick += gcnew System::EventHandler(this, &malysh::GameForm::timer_chk_Tick);
}
inline Void GameForm::timer_chk_Tick(Object^ sender, EventArgs^ e)
{
	...
	for each (enemy^ mob in mobs)
	{
		if (mob && (mob->get_enem_pos()->X == pp.X && mob->get_enem_pos()->Y == pp.Y)) { GameSost = false; timer_chk->Enabled = false; msg_EndGame(0); }
	}
}
...
inline Void GameForm::init_timer_scr()
{
	...
	timer_scr->Interval = 1000;
	timer_scr->Tick += gcnew System::EventHandler(this, &GameForm::timer_scr_Tick);
}
...
inline Void GameForm::init_game()
{
	...
	if (lvl == 1) { e_cnt = 3; bmb_pow = 1; bmb_cnt = 1; } //инф. о бомбах мен€етс€ только на 1-ом уровне
	if (lvl == 2) e_cnt = 4;
	if (lvl == 3) e_cnt = 4;
